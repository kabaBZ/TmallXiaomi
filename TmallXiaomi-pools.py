...
def parse(page_text,x):
    ...
    for i in page_text:
        map_list = []
        dic_title = ('.*? >(.*?) </a>                            <div.*?', i)
        dic_price = ('.*?.discntPrice:  (.*?)     -->.*?', i)
        dic_sals_num = ('.*?"sale-num(.*?)</span>.*?', i)
        dic_product_url = ('.*?href=(.*?)" target=.*?', i)
        map_list.append(dic_title)
        map_list.append(dic_price)
        map_list.append(dic_sals_num)
        map_list.append(dic_product_url)
        dic['num'] = num+(60*x)
        num += 1
        list1 = pool.map(multi_run_wrapper , map_list )
        dic['title'] = list1[0][0]
        dic['price'] = list1[1][0]
        dic['sales_num'] = list1[2][0].split('>')[1]
        dic['product_url'] = list1[3][0].split('//')[1].split('\\')[0]
        print(dic['price'])
...
f = open('data_pool.csv', 'a', encoding='utf-8')
